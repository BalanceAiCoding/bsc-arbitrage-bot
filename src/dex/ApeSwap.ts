...
// ApeSwap Router ABI（简化版，与 PancakeSwap V2 兼容）
const ROUTER_ABI = [
 ...
 * ApeSwap DEX 连接器
 */
export class ApeSwapConnector implements IDEXConnector {
  name = "ApeSwap";
  
  private router: ethers.Contract;
  private factory: ethers.Contract;
  private provider: ethers.Provider;
  private feeBps: number = 20; // 0.2%
  ...
  async getReserves(
    ...
    try {
      ...
      logger.error("Failed to get ApeSwap reserves", { 
        tokenA, 
        tokenB, 
  ...
  async getAmountOut(
    ...
    try {
      ...
      logger.error("Failed to get ApeSwap amount out", { 
        tokenIn, 
        tokenOut, 
        error: (error as Error).message 
      });
